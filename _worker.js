...
async function sendVerificationEmail(to, token, env) {
  ...
  if (env.RESEND_API_KEY && env.RESEND_API_KEY !== 'your-resend-api-key') {
    try {
      const response = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { 'Authorization': `Bearer ${env.RESEND_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: `招聘信息网 <noreply@${env.RESEND_DOMAIN || 'resend.dev'}>`, to: [to], subject: '验证你的邮箱 - 招聘信息网', html }) });
...
async function sendPaymentConfirmation(to, orderId, env) {
  ...
  if (env.RESEND_API_KEY && env.RESEND_API_KEY !== 'your-resend-api-key') {
    try { await fetch('https://api.resend.com/emails', { method: 'POST', headers: { 'Authorization': `Bearer ${env.RESEND_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: `招聘信息网 <noreply@${env.RESEND_DOMAIN || 'resend.dev'}>`, to: [to], subject: '付款确认 - 招聘信息网', html }) }); } catch (e) {}
  }
  return { success: true };
}
...
async function notifyAdminNewPayment(userEmail, orderId, env) {
  ...
  if (env.RESEND_API_KEY && env.RESEND_API_KEY !== 'your-resend-api-key') {
    try { await fetch('https://api.resend.com/emails', { method: 'POST', headers: { 'Authorization': `Bearer ${env.RESEND_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: `招聘信息网 <noreply@${env.RESEND_DOMAIN || 'resend.dev'}>`, to: [adminEmail], subject: `新支付待审核 - ${userEmail}`, html }) }); } catch (e) {}
  }
  return { success: true };
}
