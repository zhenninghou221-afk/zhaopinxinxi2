...
function initNavbar() {
  ...
  if (api.isLoggedIn()) {
    const user = api.user || {};
    navbarEl.innerHTML = `
      <a href="/登陆注册/dashboard">📋 完整信息</a>
      <a href="/登陆注册/payment">💳 订阅管理</a>
    ...
    navbarEl.innerHTML = `
      <a href="/登陆注册/login">登录</a>
      <a href="/登陆注册/register" class="btn-nav">免费注册</a>
    `;
  }
}
    ...
    if (isFull) {
      ...
      return `
        <div class="row">
          ...
          <span class="row-actions">
            <a class="btn-locked" href="/登陆注册/login">🔒 登录查看</a>
            ${row.website_url ? `<a class="btn-outline" href="${escapeHtml(row.website_url)}" target="_blank" rel="noopener">${escapeHtml(row.website_text || '网址')}</a>` : ''}
          </span>
        </div>`;
    }
...
async function loadDashboard() {
  if (!api.isLoggedIn()) {
    window.location.href = '/登陆注册/login';
    return;
  }
  ...
  if (subResult.success) {
    ...
    if (!subResult.data.hasAccess && subResult.data.status !== 'none') {
      ...
      window.location.href = '/登陆注册/payment';
      return;
    }
  }
...
function renderSubscriptionBanner(sub) {
  ...
  if (sub.status === 'trial') {
    bannerEl.innerHTML = `<div class="banner trial">🎁 免费试用中 · ${sub.remaining} · <a href="/登陆注册/payment">立即订阅（¥9.90/3个月）</a></div>`;
  } else if (sub.status === 'active') {
    bannerEl.innerHTML = `<div class="banner active">✅ 订阅有效 · ${sub.remaining} · <a href="/登陆注册/payment">续费叠加</a></div>`;
  } else if (sub.status === 'trial_expired') {
    bannerEl.innerHTML = `<div class="banner expired">⏰ 免费试用已过期 · <a href="/登陆注册/payment">立即订阅 ¥9.90/3个月</a></div>`;
  } else if (sub.status === 'expired') {
    bannerEl.innerHTML = `<div class="banner expired">⏰ 订阅已过期 · <a href="/登陆注册/payment">续费 ¥9.90/3个月</a></div>`;
  } else {
    bannerEl.innerHTML = `<div class="banner preview">👋 新用户注册即享<strong>1天免费试用</strong> · <a href="/登陆注册/register">免费注册</a></div>`;
  }
}
